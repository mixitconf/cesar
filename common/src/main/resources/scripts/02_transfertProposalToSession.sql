...
INSERT INTO SESSION
	(DTYPE, ADDEDAT,DESCRIPTION,FEEDBACK,FORMAT,IDEAFORNOW,LANG,LEVEL,MAXATTENDEES,
		...
		FROM PROPOSAL p
		WHERE p.STATUS='ACCEPTED' and p.ID=2682;
...
SELECT p.ID, s.ID FROM PROPOSAL p inner join SESSION s on s.TITLE=p.TITLE and p.EVENT_ID=6 WHERE p.STATUS='ACCEPTED' and p.ID=2682;
