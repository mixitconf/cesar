...
public interface ProposalRepository extends CrudRepository<Proposal, Long> {
    ...
    @Query(value = "SELECT DISTINCT s FROM Proposal s left join fetch s.speakers sp left join fetch s.votes where s.event = :event")
    List<Proposal> findAllProposalsWithVotes(@Param("event") Event event);
