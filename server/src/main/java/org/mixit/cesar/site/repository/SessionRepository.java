...
public interface SessionRepository extends CrudRepository<Session, Long> {
    ...
    @Query(value = "SELECT DISTINCT s FROM Session s left join fetch s.interests i left join fetch s.speakers sp left join fetch s.votes left join fetch s.slot where s.event.id = :idEvent and s.valid = true and s.sessionAccepted = true and s.format <> 'LightningTalk'")
    List<Session> findAllAcceptedSessions(@Param("idEvent") Long idEvent);

    @Cacheable(CACHE_SESSION)
    @Query(value = "SELECT DISTINCT s FROM Keynote s left join fetch s.interests i left join fetch s.speakers sp left join fetch s.votes left join fetch s.slot where s.event.id = :idEvent and s.valid = true and s.sessionAccepted = true")
    List<Keynote> findAllAcceptedKeynotes(@Param("idEvent") Long idEvent);

    @Cacheable(CACHE_SESSION)
    @Query(value = "SELECT DISTINCT s FROM Talk s left join fetch s.interests i left join fetch s.speakers sp left join fetch s.votes left join fetch s.slot where s.event.id = :idEvent and s.valid = true and s.sessionAccepted = true")
