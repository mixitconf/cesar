...
public interface AccountRepository extends CrudRepository<Account, Long> {
    ...
    @Query(value = "SELECT u FROM Account u left join fetch u.authorities aut where u.token=:token")
    Account findByToken(@Param("token") String  token);

    @Query(value = "SELECT u FROM Account u left join fetch u.authorities aut where u.login=:login")
